...
class AppConfig:
    ...
    llm_backend: str = "mlx_local"  # mlx_local / remote
    ...
    mlx_llm_model: str = "mlx-community/gemma-4-e2b-it-4bit"  # 本地MLX LLM模型
    mlx_llm_max_tokens: int = 100  # 本地MLX LLM最大生成token数
    mlx_llm_temperature: float = 0.7  # 本地MLX LLM温度参数
    asr_backend: str = "mlx"
    mlx_asr_model: str = "mlx-community/Qwen3-ASR-0.6B-4bit"
    mlx_asr_language: str = "zh"
    tts_backend: str = "mlx_qwen3"
    # TTS 模型选择：kokoro / qwen3 / qwen3_8bit / qwen3_custom
    mlx_tts_model_type: str = "qwen3_custom"
    ...
    mlx_tts_qwen3_8bit_model: str = "mlx-community/Qwen3-TTS-12Hz-0.6B-Base-8bit"
    mlx_tts_qwen3_custom_model: str = "mlx-community/Qwen3-TTS-12Hz-0.6B-CustomVoice-4bit"
    mlx_tts_language: str = "zh"
    mlx_tts_voice: str = "Serena"
...
def load_config() -> AppConfig:
    ...
    # TTS 模型类型选择
    mlx_tts_model_type = os.getenv("MLX_TTS_MODEL_TYPE", "qwen3_custom").strip().lower()
    if mlx_tts_model_type not in {"kokoro", "qwen3", "qwen3_8bit", "qwen3_custom"}:
        mlx_tts_model_type = "qwen3_custom"

    # LLM后端选择
    llm_backend = os.getenv("LLM_BACKEND", "mlx_local").strip().lower() or "mlx_local"
    if llm_backend not in {"remote", "mlx_local"}:
        llm_backend = "mlx_local"

    # 本地MLX LLM模式下，API key是可选的
    gemini_api_key = os.getenv("GEMINI_API_KEY", "")
    if llm_backend == "remote" and not gemini_api_key:
    ...
    return AppConfig(
        ...
        mlx_llm_model=os.getenv("MLX_LLM_MODEL", "mlx-community/gemma-4-e2b-it-4bit"),
        mlx_llm_max_tokens=_int("MLX_LLM_MAX_TOKENS", 100),
        mlx_llm_temperature=float(os.getenv("MLX_LLM_TEMPERATURE", "0.7")),
        ...
        mlx_tts_model_type=mlx_tts_model_type,  # kokoro / qwen3 / qwen3_8bit / qwen3_custom
        ...
        mlx_tts_qwen3_8bit_model=os.getenv("MLX_TTS_QWEN3_8BIT_MODEL", "mlx-community/Qwen3-TTS-12Hz-0.6B-Base-8bit"),
        mlx_tts_qwen3_custom_model=os.getenv("MLX_TTS_QWEN3_CUSTOM_MODEL", "mlx-community/Qwen3-TTS-12Hz-0.6B-CustomVoice-4bit"),
        mlx_tts_language=os.getenv("MLX_TTS_LANGUAGE", "zh"),
        mlx_tts_voice=os.getenv("MLX_TTS_VOICE", "Serena"),
