...
def _build_mock_config() -> AppConfig:
    return AppConfig(
        ...
        mlx_llm_model="mlx-community/gemma-4-e2b-it-4bit",
        mlx_llm_max_tokens=100,
        mlx_llm_temperature=0.7,
        ...
        mlx_tts_qwen3_8bit_model="mlx-community/Qwen3-TTS-12Hz-0.6B-Base-8bit",
...
def _build_tts(config: AppConfig, backend: str):
    ...
    if config.tts_backend == "mlx_qwen3":
        ...
        model_map = {
            ...
            "qwen3_8bit": config.mlx_tts_qwen3_8bit_model,
            "qwen3_custom": config.mlx_tts_qwen3_custom_model,
        }
        selected_model = model_map.get(config.mlx_tts_model_type, config.mlx_tts_qwen3_custom_model)
...
def build_app() -> dict:
    ...
    if backend == "mock":
        ...
        # 根据配置选择LLM后端
        if config.llm_backend == "mlx_local":
            agent = MLXLLMAdapter(
                model=config.mlx_llm_model,
                max_tokens=config.mlx_llm_max_tokens,
                temperature=config.mlx_llm_temperature,
                system_prompt=config.llm_system_prompt,
            )
            llm_provider = config.mlx_llm_model
        else:
            agent = GeminiAdapter(
...
def _preload_models(app: dict, printer=None) -> None:
    ...
    if config.llm_backend == "mlx_local":
        try:
            if hasattr(app["agent"], "_load_model"):
                app["agent"]._load_model()
                printer(f"LLM model preloaded: {config.mlx_llm_model}")
        except Exception as exc:
            printer(f"Warning: LLM model preload skipped: {exc}")
...
def _describe_tts_model(app: dict) -> str:
    ...
    if app["tts_provider"] == "mlx_qwen3":
        ...
        return model_map.get(config.mlx_tts_model_type, config.mlx_tts_qwen3_custom_model)
...
def start_audio_input(app: dict) -> None:
    """Start audio capture. Records all audio frames without VAD filtering."""
...
async def _run_push_to_talk(app: dict) -> None:
    ...
    with key_reader:
        while True:
            # 清理 stdin 缓冲区，防止之前的按键影响当前轮次
            key_reader.drain_stdin()
            ...
            # 保存录音到文件（用于调试）
            turn_counter[0] += 1
            audio_save_path = f"debug_recording_{turn_counter[0]}.wav"
            try:
                import wave
                with wave.open(audio_save_path, "wb") as wf:
                    wf.setnchannels(1)
                    wf.setsampwidth(2)
                    wf.setframerate(app["config"].audio_sample_rate)
                    wf.writeframes(chunk)
                print(f"[Audio saved: {audio_save_path}]")
